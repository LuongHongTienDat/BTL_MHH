...
def problem_1():
    ...
    if free.token + busy.token + docu.token > 10: print("This program can only convert Petri Net to Transition System if all of tokens less than or equal 10!!!\n")
    else:
        str = input("Do you want to see Transition System? (Y/N)\n")
        if (str == "Y"): TS(free.token + busy.token + docu.token)
